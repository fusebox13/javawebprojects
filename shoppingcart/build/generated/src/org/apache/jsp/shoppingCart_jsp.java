package org.apache.jsp;

import javax.servlet.*;
import javax.servlet.http.*;
import javax.servlet.jsp.*;
import shopping.*;

public final class shoppingCart_jsp extends org.apache.jasper.runtime.HttpJspBase
    implements org.apache.jasper.runtime.JspSourceDependent {

  private static final JspFactory _jspxFactory = JspFactory.getDefaultFactory();

  private static java.util.List<String> _jspx_dependants;

  static {
    _jspx_dependants = new java.util.ArrayList<String>(1);
    _jspx_dependants.add("/includes/shoppingRow.jsp");
  }

  private org.glassfish.jsp.api.ResourceInjector _jspx_resourceInjector;

  public java.util.List<String> getDependants() {
    return _jspx_dependants;
  }

  public void _jspService(HttpServletRequest request, HttpServletResponse response)
        throws java.io.IOException, ServletException {

    PageContext pageContext = null;
    HttpSession session = null;
    ServletContext application = null;
    ServletConfig config = null;
    JspWriter out = null;
    Object page = this;
    JspWriter _jspx_out = null;
    PageContext _jspx_page_context = null;

    try {
      response.setContentType("text/html;charset=UTF-8");
      pageContext = _jspxFactory.getPageContext(this, request, response,
      			null, true, 8192, true);
      _jspx_page_context = pageContext;
      application = pageContext.getServletContext();
      config = pageContext.getServletConfig();
      session = pageContext.getSession();
      out = pageContext.getOut();
      _jspx_out = out;
      _jspx_resourceInjector = (org.glassfish.jsp.api.ResourceInjector) application.getAttribute("com.sun.appserv.jsp.resource.injector");

      out.write("\n");
      out.write("<!DOCTYPE html>\n");
      out.write("\n");
      out.write(" \n");

    MyCart myCart = MyCart.update(pageContext);

      out.write("\n");
      out.write("<html>\n");
      out.write("    <head>\n");
      out.write("        <meta http-equiv=\"Content-Type\" content=\"text/html; charset=UTF-8\">\n");
      out.write("        <title>Shopping Cart</title>\n");
      out.write("    </head>\n");
      out.write("    <body>\n");
      out.write("        <h1>ShoppingCart</h1>\n");
      out.write("        <h3>");
      out.print( myCart.getErrorMessage() );
      out.write("</h3>\n");
      out.write("        <table border=\"4\">\n");
      out.write("            <tr><th>Item</th><th>Price</th><th>Increment</th><th>Action</th><th>Total #</th><th>Total $</th></tr>\n");
      out.write("           \n");
      out.write("            \n");
      out.write("            ");

                    CartItem cart = new CartItem("Pants", "A", 25.99, 0);
                 
      out.write("\n");
      out.write("                ");
      out.write("<tr>\n");
      out.write("                <form action=\"shoppingCart.jsp\">\n");
      out.write("                    <td><input type=\"text\" name=\"description\" value=\"");
      out.print(cart.getDescription());
      out.write("\" readonly /></td>\n");
      out.write("                    <td><input type=\"text\" name=\"price\" value=\"");
      out.print(cart.getPrice());
      out.write("\" readonly /></td>\n");
      out.write("                    <td><input type=\"text\" name=\"increment\" /></td>\n");
      out.write("                    <td>\n");
      out.write("                        <input type=\"hidden\" name=\"itemId\" value=\"");
      out.print(cart.getItemId());
      out.write("\" />\n");
      out.write("                        <input type=\"submit\" name=\"action\" value=\"add\" />\n");
      out.write("                        <input type=\"submit\" name=\"action\" value=\"remove\" />\n");
      out.write("                    </td>\n");
      out.write("                </form>\n");
      out.write("                <td>");
      out.print( myCart.getCartTotalNum(cart.getItemId()));
      out.write("</td>\n");
      out.write("                <td>");
      out.print( myCart.getCartValue(cart.getItemId()));
      out.write("</td>\n");
      out.write("            </tr>");
      out.write("\n");
      out.write("                 ");

                    cart = new CartItem("Shirts", "B", 19.99, 0);
                 
      out.write("\n");
      out.write("                ");
      out.write("<tr>\n");
      out.write("                <form action=\"shoppingCart.jsp\">\n");
      out.write("                    <td><input type=\"text\" name=\"description\" value=\"");
      out.print(cart.getDescription());
      out.write("\" readonly /></td>\n");
      out.write("                    <td><input type=\"text\" name=\"price\" value=\"");
      out.print(cart.getPrice());
      out.write("\" readonly /></td>\n");
      out.write("                    <td><input type=\"text\" name=\"increment\" /></td>\n");
      out.write("                    <td>\n");
      out.write("                        <input type=\"hidden\" name=\"itemId\" value=\"");
      out.print(cart.getItemId());
      out.write("\" />\n");
      out.write("                        <input type=\"submit\" name=\"action\" value=\"add\" />\n");
      out.write("                        <input type=\"submit\" name=\"action\" value=\"remove\" />\n");
      out.write("                    </td>\n");
      out.write("                </form>\n");
      out.write("                <td>");
      out.print( myCart.getCartTotalNum(cart.getItemId()));
      out.write("</td>\n");
      out.write("                <td>");
      out.print( myCart.getCartValue(cart.getItemId()));
      out.write("</td>\n");
      out.write("            </tr>");
      out.write("\n");
      out.write("            \n");
      out.write("                ");

                    cart = new CartItem("Socks", "C", 5.99, 0);
                 
      out.write("\n");
      out.write("                ");
      out.write("<tr>\n");
      out.write("                <form action=\"shoppingCart.jsp\">\n");
      out.write("                    <td><input type=\"text\" name=\"description\" value=\"");
      out.print(cart.getDescription());
      out.write("\" readonly /></td>\n");
      out.write("                    <td><input type=\"text\" name=\"price\" value=\"");
      out.print(cart.getPrice());
      out.write("\" readonly /></td>\n");
      out.write("                    <td><input type=\"text\" name=\"increment\" /></td>\n");
      out.write("                    <td>\n");
      out.write("                        <input type=\"hidden\" name=\"itemId\" value=\"");
      out.print(cart.getItemId());
      out.write("\" />\n");
      out.write("                        <input type=\"submit\" name=\"action\" value=\"add\" />\n");
      out.write("                        <input type=\"submit\" name=\"action\" value=\"remove\" />\n");
      out.write("                    </td>\n");
      out.write("                </form>\n");
      out.write("                <td>");
      out.print( myCart.getCartTotalNum(cart.getItemId()));
      out.write("</td>\n");
      out.write("                <td>");
      out.print( myCart.getCartValue(cart.getItemId()));
      out.write("</td>\n");
      out.write("            </tr>");
      out.write("\n");
      out.write("                           \n");
      out.write("                ");

                    cart = new CartItem("M & Ms", "D", 0.85, 0);
                 
      out.write("\n");
      out.write("                ");
      out.write("<tr>\n");
      out.write("                <form action=\"shoppingCart.jsp\">\n");
      out.write("                    <td><input type=\"text\" name=\"description\" value=\"");
      out.print(cart.getDescription());
      out.write("\" readonly /></td>\n");
      out.write("                    <td><input type=\"text\" name=\"price\" value=\"");
      out.print(cart.getPrice());
      out.write("\" readonly /></td>\n");
      out.write("                    <td><input type=\"text\" name=\"increment\" /></td>\n");
      out.write("                    <td>\n");
      out.write("                        <input type=\"hidden\" name=\"itemId\" value=\"");
      out.print(cart.getItemId());
      out.write("\" />\n");
      out.write("                        <input type=\"submit\" name=\"action\" value=\"add\" />\n");
      out.write("                        <input type=\"submit\" name=\"action\" value=\"remove\" />\n");
      out.write("                    </td>\n");
      out.write("                </form>\n");
      out.write("                <td>");
      out.print( myCart.getCartTotalNum(cart.getItemId()));
      out.write("</td>\n");
      out.write("                <td>");
      out.print( myCart.getCartValue(cart.getItemId()));
      out.write("</td>\n");
      out.write("            </tr>");
      out.write("\n");
      out.write("            \n");
      out.write("        </table>\n");
      out.write("            \n");
      out.write("            \n");
      out.write("            \n");
      out.write("        <hr>\n");
      out.write("        <h3>Total Value in the Cart is ");
      out.print( myCart.getTotalMyCartValue() );
      out.write("</h3>\n");
      out.write("        <table border=\"3\">\n");
      out.write("            <tr><th>Item</th><th>Price</th><th>Total #</th><th>Total $</th></tr>\n");
      out.write("        ");

            for (int i=0; i < myCart.size(); i++)
            {
                CartItem ci = myCart.getCartItem(i);
                // Returning to Html Mode
        
      out.write("\n");
      out.write("            <tr>\n");
      out.write("                <td>");
      out.print(ci.getDescription() );
      out.write("</td>\n");
      out.write("                <td>");
      out.print(ci.getPrice() );
      out.write("</td>\n");
      out.write("                <td>");
      out.print(ci.getTotalNumber() );
      out.write("</td>\n");
      out.write("                <td>");
      out.print(String.format("$%.2f",   ci.getTotalValue()) );
      out.write("</td>\n");
      out.write("            </tr>\n");
      out.write("        ");

                // Returning to Java Mode
            }
         
      out.write("\n");
      out.write("        </table>\n");
      out.write("    </body>\n");
      out.write("</html>");
    } catch (Throwable t) {
      if (!(t instanceof SkipPageException)){
        out = _jspx_out;
        if (out != null && out.getBufferSize() != 0)
          out.clearBuffer();
        if (_jspx_page_context != null) _jspx_page_context.handlePageException(t);
        else throw new ServletException(t);
      }
    } finally {
      _jspxFactory.releasePageContext(_jspx_page_context);
    }
  }
}
